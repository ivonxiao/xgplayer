...
import VideoDecodeController from '../decoder/video/VideoController'
...
export default class VideoRenderFromVideo extends VideoBaseRender {
  ...
  _onError (e, firstEmit) {
    ...
    // 如果第一个分段解码失败就报错
    if (!firstEmit) {
      this._emitTimelineEvents(Events.TIMELINE.PLAY_EVENT, VIDEO_EVENTS.ERROR, this._assembleErr(e && e.message))
    }
  }
    ...
    if (this._inChaseFrame) {
      ...
      if (this.chaseFrameAfter === 1 && this._chaseToFrame) {
        ...
        if (this._frameQueue.length) {
          const frames = this._frameQueue.frames
  ...
  _setMetadata (meta) {
    logger.warn(this.TAG, 'video set metadata', meta)
    this._meta = meta
  ...
  _appendChunk (videoTrack) {
    if (!this._meta) {
      this._setMetadata(videoTrack)
    }

    const samples = videoTrack.samples.slice()
    ...
    for (let i = 0; i < len; i++) {
      ...
      this._timeRange.append([sample], this.noAudio, false)
    }
  }
  ...
  _render (force) {
    const frame = this._frameQueue.nextFrame()
    const preciseVideoDts = this.preciseVideoDts
    if (!frame || !this._timeRange) {
      ...
      const _videoDecodeController = this._videoDecodeController
      ...
      if (!this._frameQueue.length && _videoDecodeController.isIDLE) {
        this._ready = false
        const audioRender = this._parent.audioRender
    ...
    const _renderDelay = info.dts - preciseVideoDts
    ...
    if (_renderDelay < -150) {
      ...
      const frames = this._frameQueue.frames
      const { deleteFrames, useFrames } = this._getDeleteFrames(frames, preciseVideoDts)
    ...
    if (!force) {
      if (this.lastTime) {
        const delay = preciseVideoDts - this.lastPrecise
        const sDelay = Date.now() - this.lastTime
