...
export default class VideoDecoder extends EventEmitter {
  ...
  _createCanvasList () {
    ...
    for (let i = 0; i < CANVAS_COUNT; i++) {
      const { dom, ctx } = this._createCanvas()
  ...
  _getFirstFrame () {
    ...
    const currentTime = this._vCurrentTime
    const pts = this._basePTS + currentTime
    const { imageData } = this._getImageData(pts)
    this.firstEmit = true
  ...
  reStartDecode () {
    this._video.pause()
    this._reset()
    const blobUrl = this._currentBlob
    const dts = this._basePTS
    if (this._framId) {
      window.cancelAnimationFrame(this._framId)
      this._framId = null
    }
  ...
  doSeek (time) {
    if (this._isInDecoding) {
      ...
      const seekTime = this._vCurrentTime / 1000 + time >= this._vDuration / 1000 ? this._vDuration / 1000 : this._vCurrentTime / 1000 + 1
