...
import { MP4Remuxer } from 'xgplayer-transmuxer'
...
export default class VideoDecoderController extends EventEmitter {
  ...
  _bindEvents () {
    ...
    this._decoder.on('fps', (...args) => {
      // logger.log('dtime')
      const pars = [this._metaFPS, ...args, this._frameLengthPerFragment]
      this.emit('fps', ...pars)
    })
  ...
  _onRemuxed (mp4) {
    const blobUrl = this._createBlobURL(mp4)
    const info = {
      blobUrl,
  ...
  _onDecoded (imageData, pts) {
    ...
    const frameInfo = this._buildFrameInfo(imageData, pts)
    this._parent.emit(Events.DECODE_EVENTS.DECODED, frameInfo)
    this._lastPts = pts
  }

  _onFirstFrame (frame, pts) {
    const frameInfo = this._buildFrameInfo(frame, pts)
  ...
  _reduceResolution (decodeFPS) {
    ...
    if (this._currentLevel >= this._endLevel) {
      logger.log(this.TAG, '_reduceResolution', 'trigger DECODE_LOW_FPS', this._currentLevel, this._endLevel)
      this._parent.emit(Events.VIDEO.DECODE_LOW_FPS)
      return
    }
    ...
    if (this._minFPSTime > 2) {
      ...
      if (levelInfo) {
        ...
        if (this._currentLevel === 4) {
          logger.log(this.TAG, '_reduceResolution', 'trigger DECODE_LOW_FPS, _currentLevel: ', this._currentLevel)
          this._parent.emit(Events.VIDEO.DECODE_LOW_FPS)
          return
        }
  ...
  _buildFrameInfo (rgb, pts) {
    try {
      ...
      const yLinesize = width
      const uvLinesize = width / 2
      const msg = 'DECODED'
      const info = {
        baseDts: undefined,
        dts: pts,
  ...
  _shiftBlob () {
    const blobInfo = this._blobURLList.shift()
  ...
  _handleKeyFrameVideo (pts) {
    try {
      ...
      this._parent._timeRange.clean()
      if (!samples.length) {
        return
      }
      // let samples = videoTrack.samples || []
      const startDts = samples[0].dts
      const endDts = samples[samples.length - 1].dts
      const delay = startDts - this.endDts
      ...
      // 每个gop小于2s，则不remuxe
      const gopDuration = endDts - startDts
  ...
  _createBlobURL (buffer) {
    ...
    const blob = new window.Blob([buffer.buffer], {
      type: 'video/mp4'
    })
  ...
  checkMinFrames (num) {
    if (typeof num === 'number') {
      ...
      if (!this._isDecodePause) {
        const playPromise = this._decoder.play()
  ...
  frameRendend (imageObj) {
    if (imageObj.dom) {
      ...
      const canvasList = this._decoder._canvasList
      const length = canvasList.length
  ...
  init (meta) {
    ...
    this._metaFPS = (meta.fpsNum / meta.fpsDen) || 25
    this._videoWidth = meta.width
    this._videoHeight = meta.height
  ...
  startRemux (samples) {
    ...
    if (samples.length) {
      const videoTrack = this._parent._meta
      videoTrack.samples = samples
      const { videoSegment } = this._remux.remux(videoTrack)
      this._onRemuxed(videoSegment)
    } else {
  ...
  empty () {
    logger.log('empty')
    if (this._decoder) {
      const list = this._decoder._canvasList
